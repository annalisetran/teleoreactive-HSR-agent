...
class ForceSensorCapture(object):
    ...
    def __init__(self, ft_sensor_topic):
        ...
        self._wrist_wrench_sub = rospy.Subscriber(
            self._ft_sensor_topic, WrenchStamped, self.__ft_sensor_cb, queue_size=2)
...
class luggageGrasper():
    def __init__(self):
        ...
        self._arm_controller_client = actionlib.SimpleActionClient(
            ...
            control_msgs.msg.FollowJointTrajectoryAction)
        self.point_subscriber = rospy.Subscriber('/object_grasper/bag/handle_coordinate', PointStamped,self.getTargetPoseCB)
    ...
    def getTargetPoseCB(self, pose):
        # For a robot without sub and pub

        target_pose = geometry.pose(x=pose.point.x,
                            y=pose.point.y,
                            z=pose.point.z,
                            ei=-pi/2,
                            ej=-pi/2,
                            ek=-pi/2)
        self.grab_bag(target_pose, pose)
    
    def openGraspCB(self,data):
        rospy.loginfo("im in cb")
        self.open_gripper()
    def grab_bag(self, target_pose, pose):
        rospy.loginfo("grab_bag called")
        # self.timeout += 0
        self.move_to_receive_pose()
        self.empty_force = self._forceCalculator.get_current_force()
        self.open_gripper()  

        start_weight = self.check_current_weight()
        
        self.pre_grasp_pose()
        rospy.sleep(2)
        rospy.loginfo("Approaching the bag!...")
        time = 0
        ...
        try:
            ...
            self.move_to_receive_pose()
            while abs(self.check_current_weight((1,1,0)) - start_weight) < 0.6 or time >= self.timeout:
                self.say("Please put the bag in my gripper.")
                # self.timeout += 0
                sleep(5)
                time += 5
            # return False
        # print(f"Empty weight: {self.empty_weight}")
        # print(f"Current weight: {self.check_current_weight((1,0,0))}")
        # print(f"empty - currWeight: {self.empty_weight - self.check_current_weight((1,0,0))}")       
        # print(f"currWeight - empty: {self.check_current_weight((1,0,0)) - self.empty_weight}")
        # print(f"abs: {abs(self.empty_weight - self.check_current_weight((1,0,0)))}")
        print(f"Empty: {self.empty_force[0]}")
        print(f"Current: {self._forceCalculator.get_current_force()[0]}")
        # prevWeight = abs(self.check_current_weight((1,0,0)) - self.empty_weight)
        current_force = self.empty_force[2]
        weight = self.check_current_weight((1,1,0))
        
        # while abs(self.empty_force[2] - current_force) < 0.8:
        while abs(weight - start_weight) < 0.8 or time >= self.timeout:
            
            # print(f"Empty: {self.empty_force[2]}")
            if self.trial < 3:
                self.trial += 1
                self.manip_publisher.publish("Failed")
                return None
            
            self.move_to_receive_pose()
            # print(f"Current weight: {self.check_current_weight((1,1,0))}")
            # print(f"Weight: {abs(self.check_current_weight((1,1,0)) - self.empty_force)}")
            self.say("Please put the bag in my gripper.")
            current_force = self._forceCalculator.get_current_force()[2]
            # print(f"Current: {self._forceCalculator.get_current_force()[2]}")
            sleep(5)
            weight = self.check_current_weight((1,1,0))
            time += 5
        ...
        # self.move_base_rel(-0.2, 0, 0, 6)

        self.move_to_carry_pose()
        
        rospy.loginfo("Ready for NAV!")
        
        self.manip_publisher.publish("Success")
        
    
def main():
    commander = luggageGrasper()

    grasp_point = PointStamped()
    grasp_point.header.frame_id = "base_link"
    grasp_point.header.stamp = rospy.Time.now()
    grasp_point.point.x = 0.2
    grasp_point.point.y = 0.2
    grasp_point.point.z = 0.5

    input ("============ Press `Enter` to execute a grasp with a pose goal ...")
    commander.getTargetPoseCB(grasp_point)

    print ("============ Python tutorial demo complete!")

if __name__=='__main__':
    luggageGrasper()
    # rospy.init_node("bag_grasper_node")    
    rospy.spin()
    # main()        