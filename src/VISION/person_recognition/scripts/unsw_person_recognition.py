...
class PersonRecognition():
    ...
    def init_feature_extractor(self):
        # Define the base CNN model (e.g., ResNet50)
        # base_model = ResNet50V2(
        #     input_shape=(self.crop_h, self.crop_w, 3),
        #     include_top=False,
        #     weights='imagenet',
        #     pooling="avg"
        # )
        ...
        # )         
        x = base_model.output
        x = Dense(128, activation='relu')(x)  # Output 128 dimensions'
        base_model = Model(inputs=base_model.input, outputs=x)

        return base_model   
        # global_pooling_layer = GlobalAveragePooling2D()(base_model.output)
        # feature_extractor_model = Model(inputs=base_model.input, outputs=global_pooling_layer)
        # return feature_extractor_model     
    
    def process_scene(self, in_msg):               
        ### Image Preparation
        img = self.bridge.imgmsg_to_cv2(in_msg.image, desired_encoding="bgr8")  
        ...
        for person in in_msg.people:
            ...
            if person.bbox_head.occluded: # and len(face_encodings) == 0:
                ...
                # else we return None
                poi = self.extract_person_features(img, person, face_locations, face_encodings)  
    ...
    '''
    def extract_person_features(self, img, person_msg, face_locations, face_encodings):
        # try:
    ...
    def match_person(self, poi):
        ...
        for feat in poi.features:
            if feat.feature_type != "face":
                poi.id = self.match_features(feat.feature_type, feat.encoding)         
