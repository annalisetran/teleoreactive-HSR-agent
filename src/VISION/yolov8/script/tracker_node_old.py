...
class TrackerNode():
    ...
    def __init__(self) -> None:
        ...
        self.conf_thres_person = rospy.get_param("~conf_thres_person", 0.55)
        ...
        self.maximum_detection_threshold = rospy.get_param("maximum_detection_threshold", 0.05)
    ...
    def on_detections(
        ...
        # if no objects, tracking id is none
        ...
        for bbox, cls, conf,tracking_id in zip(bounding_boxes, classes, confidence_score, tracking_ids):
            ...
            bbox3d = self.convert_bb_to_3d(
                depth_image, depth_info_msg, bbox2d)
            ...
            if class_name == "person":
                ...
                # TODO: revise this to see if need to move this to the end
                detections_msg.people.append(detection)

                hpe_img = numpy_image[y_min:y_max, x_min:x_max]
                ...
                if self.use_bp:
                    ...
                    # conversion to torch tensor (copied from original yolov7 repo, this is not needed for yolov8)
                    # bp_img = masked_img.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
                    ...
                    # inferencing
                    bp_model_results= self.bp_model.track(
                        source=masked_img,
                        conf=self.conf_thres_person,
                        iou=self.iou_thres,
                        max_det=self.max_det,
                    ...
                    for bp_bbox, bp_cls, bp_conf in zip(bp_bounding_boxes, bp_classes, bp_confidence_score):
                        ...
                        print(bp_cls)
                        # TO BE UPDATED. NOW only check for head
                ...
                for i in range(len(hpe_results.keypoints.xy[0])):
                    msg = Point32()
                    if hpe_results.keypoints.conf[0][i] >= self.conf_thres:
                        hpe_results.keypoints.xy[0][i][0] += x_min
                        hpe_results.keypoints.xy[0][i][1] += y_min
