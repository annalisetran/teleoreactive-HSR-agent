...
bool 
ForeignInterface::push_message_to(const char * msg, const char * thread_name)
{
  ThreadTableLoc tid = thread_table->LookupName(thread_name);
  if (tid == (ThreadTableLoc) -1)
    return false;
  if (!thread_table->IsValid(tid)) return false;
  std::ostringstream msgstr;
  msgstr << "p2pmsg(" << threadptr->TInfo().Symbol()
         << ", " << msg << ")";

  Thread* thread = thread_table->LookupID(tid);
  if (thread != NULL) {
    thread->MessageQueue().push_back(new PedroMessage(msgstr.str().c_str()));
    return true;
  }
  return false;
}
