...
WIDTH = NUM_SLOTS*BS32

TABLE_SIZE = SLOTS_PER_TABLE*BS32
...
TABLE_NAMES = ['table1', 'shared', 'table2']
TABLE_MAP = {'table1':0, 'shared':1, 'table2':2}

TABLE_START = [slot2pos(1) - BLOCK_SIZE4, 
               slot2pos(SLOTS_PER_TABLE+2) - BLOCK_SIZE4, 
               slot2pos(2*SLOTS_PER_TABLE+3) - BLOCK_SIZE4]

SLOT_HOLES = [0, SLOTS_PER_TABLE+1, 2*SLOTS_PER_TABLE+2, 3*SLOTS_PER_TABLE+3]
...
BLOCKS = [(1, 1, '#4894ac'),
          ...
          (6, 4, '#c8c828'),
          (7, 14, '#c8dc3c'),
          (SLOT_HOLES[1]+3, 5, '#a0c828'),
          (SLOT_HOLES[1]+5, 6, '#32c828'),
          (SLOT_HOLES[1]+7, 7, '#28c8aa'),
          (SLOT_HOLES[1]+8, 8, '#4894ac'),
          (SLOT_HOLES[1]+9, 9, '#c8c828'),
          (SLOT_HOLES[1]+10, 15, '#c8c828'),
...
TABLE_SLOTS = [range(1,SLOTS_PER_TABLE+1), 
               range(SLOTS_PER_TABLE+2, 2*SLOTS_PER_TABLE+2),
               range(2*SLOTS_PER_TABLE+3, 3*SLOTS_PER_TABLE+3)]
...
SLOT2TABLE = {}
for i,ts in enumerate(TABLE_SLOTS):
    for s in ts:
        SLOT2TABLE[s] = TABLE_NAMES[i]
...
class Arm(object):
    def __init__(self, i, x, y, app):
        ...
        if i == 1:
            self.home_slot = SLOT_HOLES[1]
            self.is_over_home_test = lambda x : x <= slot2pos(SLOT_HOLES[1])
            self.is_tracking_test = lambda x : x < 0
        else:
            self.home_slot = SLOT_HOLES[2] 
            self.is_over_home_test = lambda x : x >= slot2pos(SLOT_HOLES[2])
            self.is_tracking_test = lambda x : x > 0
        self.over_home = True
    ...
    def stop(self):
        print "arm {0}  STOP ---- going home".format(self.i)
        self.go_home()
        ...
        if self.is_animating():
            ...
            if self.ydest == TOP:
                ...
                if self.reached_dest(self.y, yd, self.yspeed):
                    ...
                    self.yspeed = 0
            self.move(dx, dy)
            if self.is_tracking_test(self.xspeed) and not self.send_tracking:
    ...
    def is_over_home_table(self):
        slot = pos2slot(self.x)
        return slot in TABLE_SLOTS[0] or slot in TABLE_SLOTS[-1]
    ...
    def hold(self, slot):
        ...
        if self.app.slots[slot] == []:
            on = SLOT2TABLE[slot]
            self.app.send_msg('[r_(holding(arm{0}, {1})), f_(on_table({1}, {2}))]'.format(self.i, self.holding.i, on))
        else:
    ...
    def release(self, slot):
        ...
        if self.app.slots[slot] == []:
            on = SLOT2TABLE[slot]
            self.app.send_msg('[f_(holding(arm{0}, {1})), r_(on_table({1}, {2}))]'.format(self.i, self.holding.i, on))
        else:
            on = str(self.app.slots[slot][-1].i)
...
class BlocksApp(object):
    ...
    def __init__(self, master):
        ...
        self.slots[SLOT_HOLES[0]] = None
        self.slots[SLOT_HOLES[1]] = None
        self.slots[SLOT_HOLES[2]] = None
        self.slots[SLOT_HOLES[3]] = None
        self.blocks = []
        for s, i, c in BLOCKS:
            b = Block(slot2pos(s), TABLE_POS-BLOCK_SIZE, i, c)
            self.blocks.append(b)
            self.slots[s].append(b)
        self.arms = [Arm(1, slot2pos(SLOT_HOLES[1]-1), TOP, self), 
                     Arm(2, slot2pos(SLOT_HOLES[2]+1), TOP, self)]
        self.client = pedroclient.PedroClient()
        self.client.register('twoArmSim')
    ...
    def mouseReleaseEvent(self, event):
        ...
        if sid == 0:
            sid = 1
        elif sid in [SLOT_HOLES[1], SLOT_HOLES[2]]:
            slotpos = slot2pos(sid)
            if slotpos < event.x - BLOCK_SIZE2:
                sid += 1
            else:
                sid -= 1
    ...
    def send_state(self):
        ...
        messages.append('fa_(on_table(_, _))')
        ...
        for i,s in enumerate(self.slots):
            if s is not None and s != []:
                for j,b in enumerate(s):
                    if j == 0:
                        messages.append('r_(on_table({0}, {1}))'.format(s[0].i, SLOT2TABLE[i]))
                    else:
            ...
            for tr in task_res:
                ...
                if task in  self.tasks:
                    if tr.args[1].get_type() == pedroclient.PObject.listtype:
                        ...
                        for r in res:
                            t = TABLE_MAP.get(r.val)
                            if t is not None:
                                self.table_colours[t] = self.tasks[task][2]
                            else:
                                arm = self.arms[int((r.val)[-1])-1]
    ...
    def process_message(self, pmsg):
        ...
        if msg.get_type() == pedroclient.PObject.structtype and \
           msg.functor.val ==  'locked_resources':
            ...
            print("actions", [str(a) for a in old_actions], [str(a) for a in actions], [str(a) for a in start_actions], [str(a) for a in stop_actions])
    ...
    def process_start_actions(self, start_actions, sender_task):
        ...
        for a in start_actions:
            ...
            cmd = a.functor.val
            if cmd == 'pickup':
                self.pickup(cmd_args[0].val, cmd_args[1].val, sender_task)
            elif cmd == 'put_on_block':
                self.putdown_on_block(cmd_args[0].val, cmd_args[1].val, 
                                      sender_task)
            elif cmd == 'put_on_table':
                self.putdown_on_table(cmd_args[0].val, cmd_args[1].val, 
                                          sender_task)
            elif cmd == 'go_home':
    ...
    def pickup(self, arm, b, task):
        arm = self.arms[int(arm[-1])-1]
        b_slot = self.get_slot(b)
        block = self.slots[b_slot][-1]
        b_y = block.y
        assert not arm.is_holding(b)
    ...
    def putdown_on_table(self, arm, table, task):
        table = TABLE2ID[table]
        arm = self.arms[int(arm[-1])-1]
        assert arm.holding
        table_slot = self.free_slot(table, arm)
        self.move_block(arm, table_slot)

    def putdown_on_block(self, arm, block, task):
        arm = self.arms[int(arm[-1])-1]
        assert arm.holding
        b_slot = self.get_slot(block)
        b_y = self.slots[b_slot][-1].y - BLOCK_SIZE
        self.move_block(arm, b_slot)
    ...
    def draw_tables(self):
        ...
        self.canvas.create_rectangle(TABLE_START[0], TABLE_POS, 
                                     TABLE_START[0]+TABLE_SIZE, 
                                     TABLE_POS+TABLE_WIDTH, 
                                     fill=colour, outline=colour)
        self.canvas.create_text(TABLE_START[0]+TABLE_SIZE/2-20,367, text="Table 1", fill='black')
        colour = TASK_COLOURS[self.table_colours[1]]
        self.canvas.create_rectangle(TABLE_START[1], TABLE_POS, 
                                     TABLE_START[1]+TABLE_SIZE, 
                                     TABLE_POS+TABLE_WIDTH, 
                                     fill=colour, outline=colour)
        self.canvas.create_text(TABLE_START[1]+TABLE_SIZE/2-20,367, text="Shared", fill='black')
        colour = TASK_COLOURS[self.table_colours[2]]
        self.canvas.create_rectangle(TABLE_START[2], TABLE_POS, 
                                     TABLE_START[2]+TABLE_SIZE, 
                                     TABLE_POS+TABLE_WIDTH, 
                                     fill=colour, outline=colour)
        self.canvas.create_text(TABLE_START[2]+TABLE_SIZE/2-20,367, text="Table 2", fill='black')
    ...
    def draw_tasks(self):
        tab1 = (task for task in self.tasks.values() if task[1] == 'table1')
        tab2 = (task for task in self.tasks.values() if task[1] == 'table2')
        pos = TABLE_START[0]
        for t in tab1:
